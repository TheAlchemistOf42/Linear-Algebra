...
Matrix::Matrix(const double p_array[], unsigned p_r, unsigned p_c){
...
Matrix::Matrix(const Matrix & orig){
    ...
    for (unsigned i = 0; i < orig.m_size; i++)
...
void Matrix::rowScalar(unsigned p_row, double p_multiplier){
    if (p_row <= m_rows && p_row >= 1){
        for (unsigned i = (p_row - 1)*m_columns; i < (p_row*m_columns); i++)
...
void Matrix::replacement(unsigned p_rowDest, unsigned p_row, double p_multiplier){
    if (p_rowDest <= m_rows && p_rowDest >= 1 &&
        ...
        p_rowDest != p_row){
        unsigned row2 = (p_row - 1)*m_columns;
        for (unsigned i = (p_rowDest - 1) * m_columns; i < (p_rowDest * m_columns);
              i++){
...
void Matrix::setRow(const double p_array[], unsigned p_size, unsigned p_rowNum){
    if (p_size == m_columns && p_rowNum >= 1 && p_rowNum <= m_rows){
        for (unsigned i = m_columns * (p_rowNum - 1); i < p_rowNum*m_columns; i++)
...
void Matrix::setColumn(const double p_array[], unsigned p_size, unsigned p_colNum){
    if (p_size == m_rows && p_colNum >= 1 && p_colNum <= m_columns)    {
        for (unsigned i = p_colNum - 1; i < m_columns*(m_rows - 1) + p_colNum;
             i += m_columns)
            m_array[i] = p_array[i / m_columns];
    }
}
...
bool Matrix::checkOperation(const Matrix & other, char oper){
    ...
    else if (oper == '*') // Matrix multiplication (Ax=b) where A is host Matrix
        return (m_columns == other.m_rows);
    else if (oper == 'a') // Augment Matrix
...
Matrix Matrix::operator*(const Matrix & p_term){
    ...
    if (m_columns == p_term.m_rows){
        unsigned productSize = m_rows * p_term.m_columns;
        double * productArray = new double[productSize];
        unsigned row = 0,
            column = 0;
        for (unsigned i = 0; i < m_rows * p_term.m_columns; i++){
            double sum = 0;
            row = i / m_columns;
            column = i % m_columns;
...
Matrix & Matrix::operator=(const Matrix & p_term){
    ...
    // Check memory, I guess
    for (unsigned i = 0; i < p_term.m_size; i++)
...
Matrix Matrix::inverse(){
    if (m_rows == m_columns){
        ...
        if (deter != 0){
            if (m_rows == 1){ // Single element
                inverse[0] = 1.0 / m_array[0];
            } else if (m_rows == 2){ // 2x2 matrix
                ...
                for (unsigned row = 0; row < m_rows; row++){
                    if (m_array[row * m_columns + column] != 0){
                        ...
                        for (unsigned i = column; i < row * m_columns + column; i += m_columns){
                            if (m_array[i] != 0){
                                invert.replacement(i / m_columns + 1, row + 1, -1.0 * m_array[i]);
                                replacement(i / m_columns + 1, row + 1, -1.0 * m_array[i]);
                            }
                        ...
                        if (max != 0) {
                            ...
                            row--;
                            // Both max and min of ones below are 0
...
void Matrix::augment(const Matrix & p_term){
    if (m_rows == p_term.m_rows){
        ...
        for (unsigned i = 0; i < m_rows; i++){
            for (unsigned m1 = 0; m1 < m_columns; m1++, c++)
                temp[c] = m_array[i*m_columns + m1];
            for (unsigned m2 = 0; m2 < p_term.m_columns; m2++, c++)
...
double Matrix::determinant(){
    ...
    if (m_rows == m_columns){
        if (m_rows == 1) // 1x1
            return m_array[0];
        else if (m_rows == 2) // 2x2 down - up, ad - cb
            return m_array[0] * m_array[3] - m_array[2] * m_array[1];
        else if (m_rows == 3) // 3x3 rule, aei + bfg + cdh - gec - hfa - idb
            ...
            for (unsigned i = 0; i < m_columns; i++){
                if (m_array[i] != 0){
                    ...
                    //deter += m_array[i] * m.determinant() * pow(-1, i / m_columns + i % m_columns);
                    deter += m_array[i] * m.determinant() * ((i / m_columns + i % m_columns) % 2 == 0 ? 1 : -1);
                }
            }
            delete[] tempArray;
            return deter;
...
void Matrix::displayMatrix(std::ostream & out){
    if (m_array == 0 || m_size == 0) return;
    for (unsigned i = 0; i < m_size; i++){
        if (i % m_columns != 0 || i == 0)
            out << std::setw(3) << m_array[i] << ' ';
        else
            out << '\n' << std::setw(3) << m_array[i] << ' ';
    }
    out << '\n';
}
